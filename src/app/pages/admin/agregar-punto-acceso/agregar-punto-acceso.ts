...
export class AgregarPuntoAcceso {
  ...
  // ── Picker de hora custom: hora / minuto / periodo ─────────────────────
  horas12   = Array.from({ length: 12 }, (_, i) => (i + 1).toString().padStart(2, '0')); // 01..12
  minutos   = Array.from({ length: 60 }, (_, i) => i.toString().padStart(2, '0'));        // 00..59
  periodos  = ['a. m.', 'p. m.'];

  pickerAbierto: string | null = null; // 'entrada' | 'salida' | 'comida_inicio' | 'comida_fin' | null
  dropdownPos: { top: number; left: number } = { top: 0, left: 0 };

  // Estado temporal de cada picker mientras está abierto
  ...
  // Convierte "HH:mm" (24h) a partes { h12, m, periodo }
  ...
  // Convierte partes { h12, m, periodo } a "HH:mm" (24h)
  ...
  togglePicker(campo: string, event: MouseEvent): void {
    ...
    setTimeout(() => {
      ...
      const dropdownAlto = this.dropdownRef.nativeElement.offsetHeight;
      const espacioAbajo = window.innerHeight - rect.bottom;

      const top = espacioAbajo >= dropdownAlto + 10
        ? rect.bottom + 6
        : Math.max(10, rect.top - dropdownAlto - 6);
  ...
  // ── Validación de horarios en vivo ────────────────────────────────────
  errorHorario(): string {
  ...
  // ── Estado simulado para la vista previa ──────────────────────────────
